        ...
        function fixSaveAndNew() {
            ...
            window.saveAndNew = function() {
                ...
                try {
                    ...
                    // 创建文件记录 - 使用与数据管理中心相同的格式
                    const fileRecord = {
                        id: 'file_' + Date.now(),
                        name: `购销单_${orderNumber}_${new Date().toLocaleDateString()}`, // 使用name而不是title
                        createdBy: userInfo.username || '访客用户', // 使用createdBy而不是creator
                        createdAt: new Date().toISOString(), // 添加createdAt
                        data: formData, // 将表单数据放在data字段中
                        type: 'purchase_order'
                    };
                                        ...
                                        <div class="file-item">
                                            <div class="file-info">
                                                ...
                                                <div class="file-meta">
                                                    ...
                                                    <span>时间: ${new Date(file.createdAt || file.timestamp).toLocaleString()}</span>
                                                    <span>购货方: ${file.data?.customer || '未填写'}</span>
                                                    <span>金额: ¥${file.data?.totalAmount || '0.00'}</span>
                                                </div>
                                            </div>
