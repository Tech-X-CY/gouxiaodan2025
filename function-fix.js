...
(function() {
    ...
    function applyMainFixes() {
        ...
        // 1. 强制修复用户登录状态显示 - 终极修复版
        function forceFixUserStatus() {
            try {
                // 多次尝试修复，确保成功
                function attemptUserStatusFix(attempt = 1, maxAttempts = 5) {
                    console.log(`🔍 第${attempt}次尝试修复用户状态...`);
                    ...
                    console.log('🔍 用户登录状态:', isUserLoggedIn);
                    
                    if (isUserLoggedIn) {
                        console.log('✅ 检测到用户已登录，用户名:', userInfo.username);
                        
                        // 更新用户信息显示
                        if (userName) {
                            userName.textContent = userInfo.username;
                            userName.innerHTML = userInfo.username; // 双重保险
                        ...
                        if (userAvatar) {
                            ...
                            userAvatar.innerHTML = avatar; // 双重保险
                        ...
                        if (userBar) {
                            ...
                            userBar.style.zIndex = '1000';
                            userBar.classList.remove('hidden', 'd-none');
                            userBar.classList.add('show', 'd-flex');
                            userBar.removeAttribute('hidden');
                            
                            // 强制覆盖任何可能的CSS隐藏
                            userBar.setAttribute('style', 'display: flex !important; visibility: visible !important; opacity: 1 !important;');
                            console.log('✅ 显示用户栏');
                        }
                        ...
                        if (loginPrompt) {
                            ...
                            loginPrompt.classList.add('hidden', 'd-none');
                            loginPrompt.classList.remove('show', 'd-block');
                            loginPrompt.setAttribute('hidden', 'true');
                            
                            // 强制覆盖任何可能的CSS显示
                            loginPrompt.setAttribute('style', 'display: none !important; visibility: hidden !important; opacity: 0 !important;');
                            console.log('✅ 隐藏登录提示');
                        }
                        ...
                        if (userBar) {
                            ...
                            userBar.classList.add('hidden', 'd-none');
                            userBar.classList.remove('show', 'd-flex');
                        }
                        ...
                        if (loginPrompt) {
                            ...
                            loginPrompt.classList.remove('hidden', 'd-none');
                            loginPrompt.classList.add('show', 'd-block');
                    ...
                    setTimeout(() => {
                        ...
                        const isStillLoggedIn = currentUserInfo && currentUserInfo.username && currentUserInfo.username.trim() !== '';
                        
                        if (isStillLoggedIn) {
                            const currentUserBar = document.getElementById('userBar');
                            const currentLoginPrompt = document.getElementById('loginPrompt');
                            
                            const userBarVisible = currentUserBar && 
                                                 (currentUserBar.style.display === 'flex' || 
                                                  currentUserBar.style.display === 'block' ||
                                                  !currentUserBar.classList.contains('hidden'));
                            
                            const loginPromptHidden = !currentLoginPrompt || 
                                                    currentLoginPrompt.style.display === 'none' ||
                                                    currentLoginPrompt.classList.contains('hidden');
                            
                            console.log('🔍 验证结果:', {
                                userBarVisible,
                                loginPromptHidden,
                                shouldRetry: !userBarVisible || !loginPromptHidden
                            });
                            
                            // 如果修复不成功且还有重试次数，则重试
                            if ((!userBarVisible || !loginPromptHidden) && attempt < maxAttempts) {
                                console.log('⚠️ 修复不完全，准备重试...');
                                setTimeout(() => attemptUserStatusFix(attempt + 1, maxAttempts), 500);
                            } else if (attempt >= maxAttempts) {
                                console.log('⚠️ 已达到最大重试次数，强制最终修复...');
                                // 最后一次强制修复
                                if (currentUserBar) {
                                    currentUserBar.setAttribute('style', 'display: flex !important; visibility: visible !important; opacity: 1 !important;');
                                }
                                if (currentLoginPrompt) {
                                    currentLoginPrompt.setAttribute('style', 'display: none !important; visibility: hidden !important; opacity: 0 !important;');
                                }
                            } else {
                                console.log('✅ 用户状态修复验证成功！');
                            }
                        }
                    }, 300);
                }
                ...
                localStorage.setItem = function(key, value) {
                    ...
                    if (key === 'userInfo') {
                        console.log('🔄 检测到userInfo变化，重新修复用户状态');
                        setTimeout(() => attemptUserStatusFix(), 100);
                    }
                };
